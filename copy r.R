...
# Extract year and make a factor
cycle_daily_df <- cycle_daily_df %>%
  mutate(year_factor = factor(year(date)))

# Plot monthly trends for each year
plot_monthly_trend <- ggplot(cycle_daily_df, 
                             aes(x = month, y = count, 
                                 color = year_factor, 
                                 group = year_factor)) +
  geom_smooth(method = "loess", se = FALSE, linewidth = 1.2) +
  labs(
    title = "Monthly Cycling Trends by Year (2020-2025)",
    x = "Month",
...
# Helps check for constant variance (homoscedasticity)
p1 <- ggplot(m0_diag, aes(x = .fitted, y = .resid)) +
...
# Plot 2: Normal Q-Q
# Helps check if the errors are normally distributed
p2 <- ggplot(m0_diag, aes(sample = .resid)) +
  stat_qq(alpha = 0.5) +
...
# 1. Use augment() on the M1 model object
m1_diag <- augment(m1)

# 2. Residuals vs Fitted for M1
p1_m1 <- ggplot(m1_diag, aes(x = .fitted, y = .resid)) +
  geom_point(alpha = 0.4) +
  geom_hline(yintercept = 0, linetype = "dashed", color = "red") +
  geom_smooth(method = "loess", se = FALSE, color = "blue") +
  labs(title = "M1: Residuals vs Fitted",
       x = "Fitted Values (Predicted Count)",
       y = "Residuals") +
  theme_minimal()

# 3. Normal Q-Q Plot for M1
